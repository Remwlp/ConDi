...
class PadCollate_without_know:
    def __init__(self, img_dim=0, edge_dim = 1, twitter_dim=2, dep_dim=3, label_dim=4, text_dim=5, img_path_dim=6, use_np=False):
        """
        Args:
        ...
        self.img_path_dim = img_path_dim

        self.use_np = use_np
        self.tokenizer = AutoTokenizer.from_pretrained('/mnt/afs/real_cyz/zzc/roberta-base',add_prefix_space=True)
        # self.tokenizer = AutoTokenizer.from_pretrained('/data3/guodiandian/model/bert-base',add_prefix_space=True)
    ...
    def pad_collate(self, batch):
        ...
        xs = torch.stack(xs)
        img_edge = list(map(lambda t: t[self.edge_dim].clone().detach(), batch))
        ...
        text_list = list(map(lambda t: t[self.text_dim], batch))
        image_list = list(map(lambda t: Image.open(t[self.img_path_dim]), batch))

        # for image graph
        # attr_img = construct_edge_attr()
        return xs, img_edge, encoded_cap, word_spans, word_len, mask_batch1, edge_cap1, gnn_mask_1, np_mask_1, labels, mask_batch_img, target_labels, twitters, text_list, image_list
